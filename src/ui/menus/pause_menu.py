...
class PauseMenu(Menu):
    ...
    def _draw_background(self, screen, sw, sh, t, lp):
        star_surf = self._get_surf('stars', sw, sh)
        for star in self._stars:
            star.draw(star_surf, t)
        screen.blit(star_surf, (0, 0))

        ray_surf = self._get_surf('rays', sw, sh)
        ...
        orb_surf = self._get_surf('orbs', sw, sh)
        for p in self._particles:
            p.draw(orb_surf, t)
        screen.blit(orb_surf, (0, 0))

        ember_surf = self._get_surf('embers', sw, sh)
        for e in self._embers:
            e.draw(ember_surf, t)
        screen.blit(ember_surf, (0, 0))

        if lp < 1.0:
            burst_surf = self._get_surf('bursts', sw, sh)
